...
actor SessionStore {
    ...
    private var toolCallChannels: [String: FlutterMethodChannel] = [:]
    ...
    func remove(_ id: String) {
        sessions[id] = nil
        toolCallChannels[id] = nil
    }

    func setToolCallChannel(_ channel: FlutterMethodChannel, for id: String) {
        toolCallChannels[id] = channel
    }

    func getToolCallChannel(for id: String) -> FlutterMethodChannel? {
        return toolCallChannels[id]
    }

    func cancelAll() {
        sessions.removeAll()
        toolCallChannels.removeAll()
    }
}
    ...
    struct FlutterTool: Tool {
        ...
        @Generable
        struct Arguments {
            @Guide(description: "JSON string with tool arguments, e.g. {\"city\":\"Boston\"}")
            var args: String
        }

        func call(arguments: Arguments) async throws -> String {

            guard let jsonData = arguments.args.data(using: .utf8),
                let argumentsDict = try? JSONSerialization.jsonObject(with: jsonData)
                    as? [String: Any]
            else {
                throw NSError(
                    domain: "FlutterTool", code: 2,
                    userInfo: [
                        NSLocalizedDescriptionKey:
                            "Invalid JSON format. Expected: {\"key\":\"value\"}"
                    ])
            }
            ...
            return try await withCheckedThrowingContinuation {
                ...
                Task { @MainActor in
                    methodChannel.invokeMethod(
                        ...
                        } else if let outputDict = result as? [String: Any],
                            let jsonData = try? JSONSerialization.data(withJSONObject: outputDict),
                            let jsonString = String(data: jsonData, encoding: .utf8)
                        {
                            ...
                            continuation.resume(
                                throwing: NSError(
                                    domain: "FlutterTool", code: 4,
                                    userInfo: [NSLocalizedDescriptionKey: "Invalid tool output"]))
                        }
                    }
                }
            }
...
@available(iOS 26.0, *)
public final class NativeAiBridgePlugin: NSObject, FlutterPlugin {

    private var store: Any?
    private lazy var channelName = "native_ai_bridge"
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(
            name: "native_ai_bridge",
            binaryMessenger: registrar.messenger())
        let instance = NativeAiBridgePlugin()
        instance.methodChannel = channel
        registrar.addMethodCallDelegate(instance, channel: channel)

        let streamChannel = FlutterEventChannel(
            name: "native_ai_bridge_stream", binaryMessenger: registrar.messenger())
    ...
    private func openSession(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        Task {
            #if canImport(FoundationModels)
                var tools: [any Tool] = []
                if let toolsData = dict["tools"] as? [[String: Any]],
                    let channel = self.methodChannel
                {
                    for toolData in toolsData {
                        if let toolName = toolData["name"] as? String,
                            var toolDescription = toolData["description"] as? String
                        {
                            let argumentsSchema = toolData["arguments"] as? [[String: Any]] ?? []

                            if !argumentsSchema.isEmpty {
                                toolDescription += "\n\nArguments (as JSON string):"
                                for arg in argumentsSchema {
                                    if let argName = arg["name"] as? String,
                                        let argType = arg["type"] as? String,
                                        let argDesc = arg["description"] as? String
                                    {
                                        let optional =
                                            (arg["isOptional"] as? Bool) == true
                                            ? " (optional)" : ""
                                        toolDescription +=
                                            "\n- \(argName) (\(argType))\(optional): \(argDesc)"
                                    }
                                }
                            }

                            let flutterTool = FlutterTool(
                                name: toolName,
                                description: toolDescription,
                                sessionId: sessionID,
                                methodChannel: channel,
                                argumentsSchema: argumentsSchema)
                            tools.append(flutterTool)
                        }
                    }
                }
                ...
                if let store = self.store as? SessionStore {
                    ...
                    if let channel = self.methodChannel {
                        await store.setToolCallChannel(channel, for: sessionID)
                    }
    ...
    private func closeSession(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        guard let dict = call.arguments as? [String: Any],
            ...
            complete(
                result,
                error: FlutterError(
                    code: "INVALID_ARGUMENTS", message: "sessionId missing", details: nil))
            return
        }
        Task {
            if let store = self.store as? SessionStore {
    ...
    private func ask(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        guard let dict = call.arguments as? [String: Any],
            ...
            complete(
                result,
                error: FlutterError(
                    code: "INVALID_ARGUMENTS", message: "prompt missing", details: nil))
            return
        }
    ...
    private func summarizeText(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        guard let dict = call.arguments as? [String: Any],
            ...
            complete(
                result,
                error: FlutterError(
                    code: "INVALID_ARGUMENTS", message: "text missing", details: nil))
            return
        }
    ...
    private func generateSuggestions(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        guard let dict = call.arguments as? [String: Any],
            ...
            complete(
                result,
                error: FlutterError(
                    code: "INVALID_ARGUMENTS", message: "context missing", details: nil))
            return
        }
        ...
        Task {
            do {
                ...
                complete(
                    result,
                    error: FlutterError(
                        code: "RUN_ERROR", message: "Fail to run prompt",
...
class StreamHandler: NSObject, FlutterStreamHandler {
    private weak var plugin: NativeAiBridgePlugin?
    private var streamingTask: Task<Void, Never>?

    init(plugin: NativeAiBridgePlugin) {
        self.plugin = plugin
    }
        ...
        -> FlutterError?
    {
        guard let plugin = self.plugin else {
            return FlutterError(
                code: "PLUGIN_DEALLOCATED", message: "The plugin instance was deallocated.",
                details: nil)
        }
        ...
        streamingTask = Task {
            do {
                ...
                if !(error is CancellationError) {
                    let flutterError = FlutterError(
                        code: "STREAM_GENERATION_ERROR", message: error.localizedDescription,
                        details: nil)
