...
class MethodChannelAppleFoundationFlutter
  ...
  final methodChannel = const MethodChannel('native_ai_bridge');

  final streamChannel = const EventChannel('native_ai_bridge_stream');

  static const Duration _defaultTimeout = Duration(seconds: 30);

  static const int _maxRetryAttempts = 3;

  final ToolHandlerManager _handlerManager = ToolHandlerManager();
  ...
  Future<dynamic> _handleMethodCall(MethodCall call) async {
    if (call.method == 'handleToolCall') {
      return await _handleToolCallFromNative(call.arguments);
    }
    return null;
  }

  Future<dynamic> _handleToolCallFromNative(dynamic arguments) async {
    try {
      Map<String, dynamic> argumentsJson;

      if (arguments is String) {
        argumentsJson = Map<String, dynamic>.from(jsonDecode(arguments) as Map);
      } else if (arguments is Map) {
        argumentsJson = Map<String, dynamic>.from(arguments);
      } else {
      ...
      final toolCall = ToolCall.fromJson(argumentsJson);

      final result = await _handlerManager.handleToolCall(toolCall);
    ...
    } on AppleFoundationException {
      rethrow;
    } catch (e) {
      throw AppleFoundationException(
        'Failed to handle tool call: ${e.toString()}',
